$(document).ready(function() {
					...
					function calc_nisab() {
						$('#nsb_thn').val(
								(parseInt($('#hrg_brs').val(), 10) || 0) * 520);
						if (parseInt($('#nsb_thn').val(), 10) < parseInt($(
								'#totalGaji').val(), 10)) {
					...
					function calc_zprf() {
						$('#byr_thn').val(
								(parseInt($('#totalGaji').val(), 10)) * 0.025);
						$('#byr_bln')
								.val(
										(parseInt($('#totalGaji').val(), 10)) * 0.025 / 12);
					}

					// hitung zakat maal pertahun & perbulan
					function calc_zmaal() {
						$('#byr_thn2').val(
								(parseInt($('#ss_pdpt').val(), 10)) * 0.025);
						$('#byr_bln2')
								.val(
										(parseInt($('#ss_pdpt').val(), 10)) * 0.025 / 12);
					}
					...
					function calc_nisab2() {
						$('#bsr_nsb').val(
								(parseInt($('#hrg_mas').val(), 10) || 0) * 85);
						if (parseInt($('#bsr_nsb').val(), 10) < parseInt($(
								'#ss_pdpt').val(), 10)) {
